...
function SoapClient({clientId, clientSecret, origin, authOrigin, stack}) {
  ...
  ['create', 'retrieve', 'update', 'delete'].forEach(method => { // for each method
    client[method] = function () {
      ...
      _getBaseUrl(client, stack, () => { // get the custom base url
...
function _getBaseUrl(client, stack, callback) {
