...
var ET_Client = function(clientId, clientSecret) {

    if (!clientId || !clientSecret) {
        throw new Error('The client clientId and clientSecret props are required.');
    }

    const {origin, authOrigin, stack, proxy} = _getOrigins(arguments);
...
function _getOrigins(args) {
    let origin, authOrigin, stack, proxy;

    // if the arguments are in the old shape: clientId, clientSecret, stack
    if (args.length === 3) {
    ...
    // else if arguments are in the shape: clientId, clientSecret, origin, authOrigin, proxy
    else {
        origin = args[2];
        authOrigin = args[3];
        proxy = args[4];
    ...
    return {origin, authOrigin, stack, proxy};
}
