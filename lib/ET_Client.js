...
var ET_Client = function() {
    ...
    const {clientId, clientSecret, origin, authOrigin, stack} = _getArguments(arguments);
...
function _getArguments(args) {
    ...
    // if the arguments are in the new shape: object
    if (args.length === 1) {
        return args[0];
    }

    // else if the arguments are in the old shape: clientId, clientSecret, stack
    else {
        const clientId = args[0];
        const clientSecret = args[1];
        const origin = 'https://www.exacttargetapis.com';
        const authOrigin = 'https://auth.exacttargetapis.com';
        const stack = args[2] || '';
        return {clientId, clientSecret, origin, authOrigin, stack};
    }
