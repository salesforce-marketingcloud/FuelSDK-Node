...
const ET_Client = function (id, secret, origin) {
    if (!id || !secret || !origin) {
        throw new Error('The client id, secret and origin are required.');
    }
    this.restClient = new RestClient(id, secret, origin);
    this.soapClient = new SoapClient(id, secret, origin);
};
