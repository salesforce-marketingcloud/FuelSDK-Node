...
function _getSoapBaseUrl(client, stack, soapOrigin, globalReqOptions, callback) {
    if (soapOrigin) { // if base url fetched, run callback
        ...
        callback();
    } else if (client.baseUrlFetched) { // if base url fetched, run callback
		callback();
...
var ET_Client = function(clientId, clientSecret, stack, origin, authOrigin, {soapOrigin, globalReqOptions = {}} = {}) {
	...
	_stack=stack;
	this.FuelAuthClient = this.initAuth(clientId, clientSecret, globalReqOptions);
	this.RestClient = this.initRest(this.FuelAuthClient, origin, authOrigin, globalReqOptions);
	this.SoapClient = this.initSoap(clientId, clientSecret, stack, origin, authOrigin, soapOrigin, globalReqOptions);

};

ET_Client.prototype.initAuth = function(id, secret, globalReqOptions) {
...
ET_Client.prototype.initRest = function(auth, origin, authOrigin, globalReqOptions) {
    var authUrl = `${authOrigin}/v1/requestToken`;
	var options = {auth, globalReqOptions};
	options.auth.authUrl = authUrl;
	options.auth.globalReqOptions = globalReqOptions;
	options.headers = globalReqOptions.headers;
	var client =  new FuelRest(options);
	client.origin = origin;
	return client;
};

ET_Client.prototype.initSoap = function(clientId, clientSecret, stack, origin, authOrigin, soapOrigin, globalReqOptions) {
    const authUrl = `${authOrigin}/v1/requestToken`;
    const client = new FuelSoap({
		auth: {clientId, clientSecret, authUrl},
    ...
    ['create', 'retrieve', 'update', 'delete'].forEach(method => { // for each method
        client[method] = function () {
            ...
            _getSoapBaseUrl(client, stack, soapOrigin, globalReqOptions, () => { // get the custom base url
